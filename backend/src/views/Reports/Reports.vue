<template>
  ...
  >
    <h2 class="font-bold mb-5 text-left">Report Chart</h2>
  ...
  axiosClient
    ...
    .then((response) => {
      ...
      if (selectedPeriod.value === "daily") {
        ...
        orderData = data.map((item) => item.total_orders);
        customerData = data.map((item) => item.total_customers);
      } else if (selectedPeriod.value === "weekly") {
        let weeklyGroups = {};
        let orderWeeklyGroups = {};
        let customerWeeklyGroups = {};
        ...
        data.forEach((item) => {
          ...
          if (!weeklyGroups[weekLabel]) {
            ...
            orderWeeklyGroups[weekLabel] = 0;
            customerWeeklyGroups[weekLabel] = 0;
          }
          orderWeeklyGroups[weekLabel] += parseFloat(item.total_orders);
          customerWeeklyGroups[weekLabel] += parseFloat(item.total_customers);
        });

        labels = Object.keys(weeklyGroups);
        orderData = Object.values(orderWeeklyGroups);
        customerData = Object.values(customerWeeklyGroups);
      } else if (selectedPeriod.value === "monthly") {
        let monthlyGroups = {};
        let orderMonthlyGroups = {};
        let customerMonthlyGroups = {};
        ...
        data.forEach((item) => {
          ...
          if (!monthlyGroups[monthLabel]) {
            ...
            orderMonthlyGroups[monthLabel] = 0; // Initialize properly
            customerMonthlyGroups[monthLabel] = 0; // Initialize properly
          }

          orderMonthlyGroups[monthLabel] += item.total_orders || 0;
          customerMonthlyGroups[monthLabel] += item.total_customers || 0;
        });

        labels = Object.keys(monthlyGroups);
        orderData = Object.values(orderMonthlyGroups);
        customerData = Object.values(customerMonthlyGroups);
      }
